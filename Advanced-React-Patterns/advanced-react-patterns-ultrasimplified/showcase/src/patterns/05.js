...
const MediumClap = ({
	...
	//  style: userStyles = {} <-- here we're catching the style props, but renaming as "userStyles", and setting an empty object if no style props are passed
	...
	}, [count, isControlled, onClap, clapState]);
...
const Usage = () => {
	...
	const handleClap = () => {
		setState((prevState) => ({
			count: Math.min(prevState.count + 1, MAXIMUM_CLAP_VALUE),
			countTotal:
				prevState.count < MAXIMUM_CLAP_VALUE
					? prevState.countTotal + 1
					: prevState.countTotal,
			isClicked: true,
		}));
