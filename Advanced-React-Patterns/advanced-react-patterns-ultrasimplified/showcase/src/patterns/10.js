...
const useClapState = (
	...
	const nonChangingInitialState = useRef(initialState);
	...
	// props collection for 'click' event
	...
	const resetRef = useRef(0); // a glorified counter 0, 1, 2, 3, ...
	const prevCount = usePrevious(count);
	const reset = useCallback(() => {
		if (prevCount !== count) {
			dispatch({ type: "reset", payload: nonChangingInitialState.current });
			resetRef.current++;
		}
	}, [prevCount, count]);
...
const ClapCointainer = ({ children, setRef, handleClick, ...restProps }) => {
	return (
		<button
			ref={setRef}
		...
		<div>
			...
			<section>
				...
				<pre className={userStyles.resetMsg}>
					<code>{JSON.stringify({ timesClapped, count, countTotal })}</code>
				</pre>
				<pre className={userStyles.resetMsg}>
				...
				<pre style={{ color: "red" }}>
					{isClappedTooMuch
						? "You have clapped too much, don\t be so generous"
						: ""}
				</pre>
			</section>
		</div>
